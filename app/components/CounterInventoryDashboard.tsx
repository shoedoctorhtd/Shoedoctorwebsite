...
export default function CounterInventoryDashboard({ initialInventory, initialHistory, canRecord }: {
  ...
  return <main id="main-content" className={`${styles.shell} ${counter.shell}`}>
    <nav className={styles.nav} aria-label="Product administration"><Link href="/admin">Dashboard</Link><Link href="/admin/products">Products</Link><Link href="/admin/inventory">Inventory history</Link><Link href="/admin/product-orders">Product orders</Link></nav>
    ...
    <div className={counter.workspace}>
      ...
      <aside className={`${styles.panel} ${counter.basket}`}><h2>Counter Sale</h2>{lines.length ? lines.map((line) => <div className={counter.line} key={line.product.id}><span>{line.product.name} × {line.quantity}<small>{money(line.product.priceNpr!)} each</small></span><strong>{money(line.quantity * line.product.priceNpr!)}</strong><button className={styles.secondary} disabled={busy} aria-label={`Remove ${line.product.name}`} onClick={() => setLines(lines.filter((item) => item.product.id !== line.product.id))}>Remove</button></div>) : <p>Add products for one counter transaction.</p>}<p className={counter.total}>Total <strong>{money(total)}</strong></p><button className={styles.primary} disabled={!canRecord || !lines.length || busy} onClick={() => { setError(""); setConfirmation([...lines]); }}>Confirm Counter Sale</button>{!canRecord && <p>Your account can view inventory. Recording a sale requires counter-sale permission.</p>}</aside>
    </div>
