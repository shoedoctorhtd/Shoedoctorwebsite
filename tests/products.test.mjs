...
test("homepage care essentials and permanent product deletion keep their public and admin boundaries", async () => {
  ...
  assert.ok(homePage.indexOf("<HomeCareEssentials products={homepageProducts} />") < homePage.indexOf('id="services-overview"'));
  assert.match(homeSection, /if \(!visibleProducts\.length\) return null/);
  assert.match(homeSection, /Shoe Doctor Care Essentials/);
  assert.match(homeSection, /Limited availability/);
  assert.match(homeSection, /formatNprOrPending/);
  assert.doesNotMatch(homeSection, /Genuine product|productBadgeLabel|product\.shortDescription|data-reveal/);
  ...
  assert.doesNotMatch(homeStyles, /scroll-snap-(?:type|align)|overflow-x:\s*auto/);
...
test("public and admin routes enforce the catalogue, image-access, SEO, header, permission, and atomic-write contracts", async () => {
  ...
  assert.match(header, /<ServicesDesktopMenu/);
  assert.match(header, /navigationItems\.services,[\s\S]*navigationItems\.products,[\s\S]*navigationItems\.about,[\s\S]*navigationItems\.contact/);
  assert.match(header, /navigationItems\.home,[\s\S]*navigationItems\.steam,[\s\S]*navigationItems\.blog,[\s\S]*navigationItems\.donate/);
  assert.match(header, /Steam Cleaning/);
